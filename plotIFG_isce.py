...
def plotEPGFZdata(GDALfilename, outfilename, title = None, aspect = 1, datamin = None, datamax = None, draw_colorbar = None, colorbar_orientation = "horizontal"):
    ds = gdal.Open(GDALfilename, gdal.GA_ReadOnly)
    slc = ds.GetRasterBand(1).ReadAsArray()
    transform = ds.GetGeoTransform()
    ds = None
    ...
    # load mask
    with h5py.File('mask_EPGFZ.h5', "r") as f:
        a_group_key = list(f.keys())[0]
        E_mask = f[a_group_key][()]
    ...
    # select EPGFZ region 
    EPGFZ = slc[5050:7520,:]
    angle_slc = np.angle(EPGFZ)
    angle_slc = angle_slc + np.pi 

    #plot
    fig = plt.figure(figsize=(18, 16))
    ax = fig.add_subplot(1,2,1)
    cax1 = ax.imshow(np.abs(EPGFZ), vmin = datamin, vmax=datamax, cmap='gray', extent=[xmin,xmax,ymin,ymax/3])
    ax.set_title(title + " (amplitude)")
    if draw_colorbar is not None:
        cbar1 = fig.colorbar(cax1,orientation=colorbar_orientation)
    ax.set_aspect(aspect)
    ...
    ax = fig.add_subplot(1,2,2)
    #cax2 = ax.imshow(np.angle(EPGFZ),cmap='rainbow',extent=[xmin,xmax,ymin,ymax/3])
    cax2 = ax.imshow(angle_slc, vmin = 0, vmax = 2*np.pi, cmap='jet', extent=[xmin,xmax,ymin,ymax/3])
    ax.set_title(title + " (phase [rad])")
    if draw_colorbar is not None:
        cbar2 = fig.colorbar(cax2,orientation=colorbar_orientation)
    ax.set_aspect(aspect)
