...
print(Counter,'\n')
numstacks = np.ceil(Counter/60).astype(int)

for i in range(numstacks):
    if i != numstacks-1 : 
        fig, axs = plt.subplots(10, 6, sharex='col', sharey='row', gridspec_kw={ 'wspace': 0}, figsize=(30,50))
        ...
        for ax in axs.flat:
            ...
            ax.text(.6,.12, line_var[0:17], fontsize = 15)
            ax.text(.9,.95, str(j-1), fontsize = 25, horizontalalignment='center', verticalalignment='center', transform=ax.transAxes)
        ...
        lines_variable = f.readlines()
        j = i*60+1 
        numlast = np.mod(Counter, 60).astype(int)
        if numlast == 0:
            numlast = 60
